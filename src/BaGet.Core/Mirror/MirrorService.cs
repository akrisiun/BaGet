        ...
        private async Task<bool> TryIndexFromSourceAsync(string id, NuGetVersion version)
        {
            ...
            try
            {
                ...
                using (var stream = await _downloader.DownloadOrNullAsync(packageUri, CancellationToken.None))
                {
                    ...
                    // Tuple<string, IndexingResult> r = await _indexer.IndexAsync(stream, CancellationToken.None);
                    var indexingResult = await _indexer.IndexAsync(stream, CancellationToken.None); // r.Item2;
